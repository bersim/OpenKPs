...
- Секция **Topic** - данная секция является дочерней для секций **MqttPubTopics** и **MqttSubTopics** и предназначена для конфигурации топика.
	...
	- Атрибут **Retain** - данный атрибут определяет поведение сервера при подписке на этот топик. Используется для секции **MqttSubTopics** .  
	...
	- Атрибут **NDS** - данный атрибут определяет разделитель десятичного разряда для чисел с плавающей точкой. Исользуется для секции  **MqttSubTopics** .  
