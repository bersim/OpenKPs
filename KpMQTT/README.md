...
- Секция **Topic** - данная секция является дочерней для секций **MqttPubTopics** и **MqttSubTopics** и предназначена для конфигурации топика.
	...
	- Атрибут **Retain** - установка данного атрибута в значение "true" позволяет получить от брокера последнее опубликованное значение.
...
<DevTemplate>
  ...
  <MqttPubTopics>
	<Topic TopicName="/myparam10" QosLevel="0" NumCnl="21" PubBehavior="OnChange" Retain="false"/>
  </MqttPubTopics>
</DevTemplate>
```
